...
$fileType = pathinfo($targetFilePath,PATHINFO_EXTENSION);//print_r($fileType);//exit;

 $allowed_image_extension = array("png","jpg","jpeg","jfif");
    
$updated_by='Gajendhrababu';

$datedddd =date('d-m-Y'); 
$date1 =  date('d-m-Y',(strtotime("+3 day")));//print_r($date1);exit;

if(isset($_POST['event_name'])){ 
	$event_name=$_POST['event_name']; 
	$event_string=str_replace(" ","_","$event_name");
	$event_organizer=$_POST['event_organizer'];
	$eventtype=$_POST['eventtype'];
	$start_date=$_POST['start_date'];
	$end_date=$_POST['end_date'];
	$frequency=$_POST['frequency']??"NA";
	$repeat_date=$_POST['repeat_date']??"na";
	$location=$_POST['location'];
	$description=$_POST['description'];
	$myfile=$_FILES['myfile'];
	if(in_array($fileType,$allowed_image_extension)){
		$insert_file=$targetDir.''.$event_name.'.'.$fileType;
	 $move=move_uploaded_file($_FILES['myfile']['tmp_name'],"$targetDir".$event_name.'.'.$fileType[1]); 
	//$move=move_uploaded_file($_FILES['myfile']['tmp_name'],$targetFilePath); 
	$insert_event_handler=pg_query($db,"INSERT INTO public.event_handler(event_name, event_organizer, event_type, start_date, end_date, repeat_date, location, description, updated_on, updated_by, frequency,event_image)VALUES ('$event_name', '$event_organizer','$eventtype','$start_date','$end_date','$repeat_date','$location','$description','now()','$updated_by','$frequency','$insert_file')");
	}else{
	...
	if($insert_event_handler){$data  = 'insert'; }else{$data  = 'notinserted';}
	
die($data);	  
}


$sel1=pg_query($db,"SELECT id, event_name, event_organizer, event_type,start_date,end_date,repeat_date, 
location, description, updated_on, updated_by, frequency, event_image FROM public.event_handler WHERE start_date > '$datedddd'  group  by start_date,id,event_name,event_organizer, event_type, end_date,repeat_date, location, description, updated_on, updated_by, frequency, event_image order by start_date desc limit 5;");




  while($fetch_event=pg_fetch_assoc($sel1)){
	
	$fetch_name=ucwords($fetch_event['event_name']); 
	$event_type=$fetch_event['event_type']; 
	$fetch_name222=$fetch_event['description']; 
	$fetch_name666=$fetch_event['frequency']; 
	$fetch_name333=date('d-m-Y',strtotime($fetch_event['start_date'])); 
	$fetch_name555=date('d-m-Y',strtotime($fetch_event['repeat_date'])); 
	$fetch_name444=date('h:i A',strtotime($fetch_event['start_date'])); 
	$directory = $targetFilePath;
	$image_path=$fetch_event['event_image'];
	$fetch_file= glob($image_path);
	foreach($fetch_file as $file_index=> $file_value){
		$get_file[]=$file_value; 
	}
	
	
	if($event_type=="special"){
		$overall[]="<img src=".$get_file." width='50' height='50'>".$fetch_name."-&nbsp;on&nbsp;".$fetch_name333."&nbsp;at&nbsp;".$fetch_name444;
	}else{
        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-lg-8">
					...
					<?php foreach($get_file as $img_key=>$img_value){
						$glob=glob($img_value);
						foreach($glob as $loo=>$kloo){?>
